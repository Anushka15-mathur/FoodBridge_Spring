...
public class AuthServiceImpl implements AuthService {
    ...
    public UserResponse getCurrentUser() {
        ...
        response.setProfileCompleted(resolveProfileCompleted(user));

        return response;
    }

    private boolean resolveProfileCompleted(User user) {
        boolean completed = user.isProfileCompleted();

        if (user.getRole() == Role.RESTAURANT) {
            completed = restaurantRepository.findByUser(user).isPresent();
        }
